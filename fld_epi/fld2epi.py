...
#  CHK		optional: preamble of CHK commands (eg for EpiData LABELBLOCK, DEFINEs, etc)
...
# 
#
# Note that QES blocks only provide info for the .QES file. CHK blocks only provide info for the .CHK file.
# FLD blocks contain info which involves writing to both QES and CHK files.
# 
# FLD blocks actually contain a mixture of line-types (sort of sub-blocks within it). FLD block must initially
# define the field name/type/width/decimals and prompt-string (eg "weight n 4 1 Height in cm"). This may then be followed
# by either:
...
while EXISTS(line):				# repeat while scan for ... ended successfully
 ...
 while EXISTS(line) and not FLDSPEC(line):	# look for start of a fldspec
  # process current
  line= line.rstrip()
  blen += 1
